...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-body">
                <form class="form w-100" novalidate="novalidate"  action="{{ route('admin.user-update',$user->id) }}" method="POST">
    @csrf
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="fv-row mb-8">
                <input type="text" placeholder="Name" name="name" autocomplete="off" class="form-control bg-transparent @error('name') is-invalid @enderror" value="{{ $user->name }}"  />
                @error('name')
        ...
        <div class="col-md-6">
            <div class="fv-row mb-8">
                <input type="text" placeholder="User name" name="username" autocomplete="off" class="form-control bg-transparent @error('username') is-invalid @enderror" value="{{  $user->username }}"  />
                @error('username')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="col-md-12">
            <div class="fv-row mb-8">
                <input type="text" placeholder="Email" name="email" autocomplete="off" class="form-control bg-transparent @error('email') is-invalid @enderror" value="{{  $user->email }}" />
                @error('email')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="fv-row mb-8">
                ...
                <select class="form-select @error('role') is-invalid @enderror" data-control="select2" data-placeholder="Select Role" name="role">
                    ...
                    @foreach($roles as $role)
                        ...
                        @endphp
                        <option value="{{$role->id}}" @if($role->name == $user->roles[0]->name) selected @endif>
                            {{ucWords($name)}}
                        </option>
                    @endforeach
                </select>
